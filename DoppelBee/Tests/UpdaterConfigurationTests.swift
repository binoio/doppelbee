...
final class UpdaterConfigurationTests: XCTestCase {
    /// Tests are app-hosted, so Bundle.main is DoppelBee.app.
    ...
    func testPublicKeyIsARealEd25519Key() throws {
        ...
        XCTAssertNotEqual(
            ...
            Back up the private key — losing it means no installed copy of DoppelBee
    ...
    func testDownloaderServiceIsNotEnabled() {
        ...
        // which DoppelBee has. Enabling it anyway adds a pointless XPC hop.
        XCTAssertNil(info["SUEnableDownloaderService"],
                     "DoppelBee has network.client, so the downloader XPC is unnecessary")
    }
    ...
    func testEntitlementsDeclareInstallerMachServices() throws {
        ...
        let entitlements = URL(fileURLWithPath: #filePath)
            ...
            .deletingLastPathComponent()   // DoppelBee
            .appendingPathComponent("Resources/DoppelBee.entitlements")
